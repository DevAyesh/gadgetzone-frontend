...
export default async function CollectionPage(props: { params: Promise<{ slug: string }> }) {
  ...
  try {
    const res = await fetch(`${API_URL}/api/collections/${slug}`, { next: { revalidate: 60 } });
    if (res.ok) {
  ...
  const pageTitle = collection?.name || slug.split('-').map((w: string) => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
            ...
            return (
              <Card key={i} className="group overflow-hidden border-border/50 bg-card hover:shadow-xl hover:shadow-primary/5 transition-all duration-300 flex flex-col">
                <Link href={`/shop/${product.slug}`} className="relative aspect-square overflow-hidden bg-muted block">
                  ...
                  <img src={primaryImage} alt={product.name} className="object-cover w-full h-full group-hover:scale-105 transition-transform duration-500" />
                </Link>
                <CardContent className="p-5 flex flex-col flex-1">
                  ...
                  <div className="mt-2 flex items-center gap-2">
                    ...
                    {product.old_price && (
                      <span className="text-sm text-muted-foreground line-through">{formatPrice(product.old_price)}</span>
                    )}
                  </div>
                  <div className="mt-auto pt-4 relative z-20">
                    <AddToCartButton product={{ id: product.id || String(i), name: product.name, price: product.price, slug: product.slug, image_url: primaryImage }} />
                  </div>
                </CardContent>
              </Card>
            );
